...
- In TT Fabric dataflow kernels, allocate packet-header CBs exactly as fabric expects: `DataFormat::RawUInt32`, page size `tt::tt_fabric::get_tt_fabric_packet_header_size_bytes()` (96 bytes on the current 2D torus route-buffer-size-35 path), and enough pages for every live header (typically two pages for distinct north/south headers). Do not reuse a one-page `UInt32` CB for fabric packet headers; it can make fabric barriers or mcasts hang.
